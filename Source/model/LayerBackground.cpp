...
void LayerBackground::LoadVolumeRenderingSource() {
	...
	long long int data_size = lvalue * volumeRenderWidth * volumeRenderHeight * volumeRenderDepth;
	long long int tile_size = lvalue * DataBlockSize * DataBlockSize;
	long long int block_size = lvalue * DataBlockSize * DataBlockSize * DataBlockSize;
	...
	for (int z = 0; z < volumeRenderDepth; z+= DataBlockSize / pow(2, level)) {
		...
		for (int y_b = 0; y_b < y_count; ++y_b) {
			for (int x_b = 0; x_b < x_count; ++x_b) {
				...
				long long int offset_size = lvalue * tile_size;
