...
void ViewHeatmapGLWidget::setHeatmapData(std::vector<cell> *cell_list_ptr, std::list<unsigned int> *indexlist_ptr, coord data_size) {
...
void ViewHeatmapGLWidget::updateHeatmap() {
	...
	for (iter = index_list->begin(); iter != index_list->end(); ++iter) {
		coord minbox = cell_table->at(*iter-1).minbox;
		coord maxbox = cell_table->at(*iter-1).maxbox;
